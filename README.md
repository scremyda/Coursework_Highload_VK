...
Также копии столбцов таблиц accommodation(title, description, price_min), accommodation_stats(rating), accommodation_type(name, description), category(name, description) можно положить в ElasticSearch, тк он прекрасно ориентирован на работу с документами. После индексации мы можем выполнять поиск, сортировать, фильтровать данные, а не строки данных в столбцах. Что, в свою очередь, демонстрирует иной подход к поиску данных, и указывает на то, что Elasticsearch может выполнять сложный полнотекстовый поиск.
...
Для эффективного поиска отелей используем ElasticSearch. 
