...
В качестве ключа для шардинга используются id(PK) путем хэширования UUID и распределению по хэшу (Согласованное хэширование). Используем данный метод для таблиц accomodation(из-за быстро доступа), book_review(из-за большого размера 5Тб+) и accomodation_type(из-за быстро доступа).

### Репликация.

Будем использовать схему Master-Slave c асинхронной репликацией. Применим данную схему на user, review_stats и accomodation_stats - т.к. нам не важна скорость записи + review_stats и accomodation_stats будут в кэше и им не нужен частый апдейт.  
