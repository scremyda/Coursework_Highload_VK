...
В качестве ключа для шардинга используются id(PK) путем хэширования UUID и распределению по хэшу (Согласованное хэширование). Используем данный метод для таблиц book_review(из-за большого размера 5Тб+), review_stats(из-за высокой нагрузки на запись) и accomodation_stats(из-за высокой нагрузки на запись).
